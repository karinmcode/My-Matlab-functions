function     outFO=mySaveAllDataAlignedToVid_m523_211110_002(url_vid,url_proc,DoSave)
...
%% MANUAL SYNC TIMES for 1st chirp

t1stchirp_sbx = 4797.73;
tlastchirp_sbx = 6737.97+2;

t1stchirp_events = 12713.175388;
t1stchirp_vidproc = 3.697;
...
if isSaved
    ...
    speed.time = continuous.encoder.time;%figure;plot(speed.time)
...
%     IDs(:,3) = str2num(rec.sbx);
   ...
   for iu = 1:numel(URLs_fluo)
    ...
    if myisfile(url_fluo)%mywinopen(fileparts(url_fluo))
        ...
        i4camfile = fluo.time>=(t1stchirp_sbx-t1stchirp_vidproc) &   fluo.time<=tlastchirp_sbx;%plot(i4camfile)
    ...
    if myisfile(url_events)
        ...
        i1stchirp = 595;
        ilastchirp = find(strcmp(events.sound.sound_categories,'chirp'),1,'last');
        t1stchirp = events.sound.time(i1stchirp);
        tlastchirp = events.sound.time(ilastchirp)+2;
        ...
        for iev = 1:nev
            ...
            i4camfile = t>=t1stchirp &   t<=tlastchirp;%plot(i4camfile)
            ...
            events.(this_ev).time_proc_vid = events.(this_ev).time-events.(this_ev).time(1)+t1stchirp_vidproc;
        end
