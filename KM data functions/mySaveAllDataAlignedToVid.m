...
if isSaved
    ...
    i4filename = strncmp(continuous.cam(camID).filename,fn.vid,numel(fn.vid));
    time_beh = continuous.cam(camID).time(i4filename);%tVID(1)
...
%     IDs(:,3) = str2num(rec.sbx);
    ...
    if ~myisfile(url_fluo)%mywinopen(fileparts(url_fluo))
        url_fluo = replace(url_fluo,'_fluo','');%mywinopen(url_fluo)
    end
    if ~myisfile(url_fluo)%mywinopen(fileparts(url_fluo))
        url_fluo = url_proc;%mywinopen(url_fluo)
    end
    if ~myisfile(url_fluo)%mywinopen(fileparts(url_fluo))
        url_fluo = replace(url_proc,'.mat','_resp_cells.mat');%mywinopen(url_fluo)
    end
