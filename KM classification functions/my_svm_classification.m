...
% Set up SVM options
if ~isfield(params,'KernelScale')
    paramsfig.KernelScale = 5;
    paramsfig.KernelFunction = 'gaussian';
else
    paramsfig.KernelScale = params.KernelScale;
    paramsfig.KernelFunction = 'gaussian';
end
...
title({'SVM Confusion matrix' , sprintf('(accuracy = %.2f)' ,accuracy.test )});
...
%% add fig params
h=myAddParams2Fig(fig,paramsfig);
