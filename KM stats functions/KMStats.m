...
if params.doplot
    ...
    if params.plotbars
        ...
        Xstars = find(issig&aboveThreshold);
        vOff  = (max(Y(:))-min(Y(:)))*0.05;
        Ystars = repmat(max(Y(:))+vOff,numel(Xstars),1);
        hold(ax,'on');
        hstars = text(ax,Xstars,Ystars,'*','BackgroundColor','none','HorizontalAlignment','center','VerticalAlignment','middle','color','k','tag','hstars');
        stats.hstars = hstars;
